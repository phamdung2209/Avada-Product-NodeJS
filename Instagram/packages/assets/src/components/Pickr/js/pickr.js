...
export default class Pickr {
  ...
  _bindEvents() {
    ...
    const eventBindings = [
      ...
      _.on([_root.interaction.cancel, _root.preview.lastColor], 'click', () => {
    ...
    if (!options.showAlways) {
      ...
      eventBindings.push(
        ...
        _.on(
          document,
          'keyup',
          e => this.isOpen() && (e.key === ck || e.code === ck) && this.hide()
        ),
        ...
        _.on(
          ...
          e => {
            if (
              this.isOpen() &&
              !_.eventPath(e).some(el => el === _root.app || el === _root.button)
            ) {
              this.hide();
            }
          },
  ...
  addSwatch(color) {
    ...
    if (values) {
      ...
      const el = _.createElementFromString(
        `<button type="button" style="--pcr-color: ${color
          .toRGBA()
